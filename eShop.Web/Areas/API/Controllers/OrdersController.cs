...
using OrderItem = eShop.Entities.OrderItem;
...
namespace eShop.Web.Areas.API.Controllers
{
    public class OrdersController : APIBaseController
    {
        ...
        public eShopSignInManager SignInManager
        {
            get
            {
                return _signInManager ?? Request.GetOwinContext().GetUserManager<eShopSignInManager>();
            }
            private set
            {
        ...
        public eShopUserManager UserManager
        {
            get
            {
                return _userManager ?? Request.GetOwinContext().GetUserManager<eShopUserManager>();
            }
            private set
            {
                _userManager = value;
        ...
        public eShopRoleManager RoleManager
        {
            get
            {
                return _roleManager ?? Request.GetOwinContext().Get<eShopRoleManager>();
            }
            private set
            {
                _roleManager = value;
        ...
        public OrdersController(eShopUserManager userManager, eShopSignInManager signInManager, eShopRoleManager roleManager)
        {
            ...
            if (model != null && ModelState.IsValid)
            {
                ...
                if (cartProducts != null && cartProducts.Count > 0)
                {
                    ...
                    else if (model.CreateAccount)
                    {
                        try
                        {
                            var user = new eShopUser { FullName = model.FullName, UserName = model.Email, Email = model.Email, PhoneNumber = model.PhoneNumber, RegisteredOn = DateTime.Now };
